...
[_BASE_MACOS] = LAYOUT_60_iso( /* Base - macOS ISO */
    ...
    MT(MOD_LSFT, JK_SB), KC_NUBS,  KC_Z,     KC_X,  KC_C,  KC_V, KC_B,    KC_N,  KC_M,  KC_COMM,  KC_DOT,               KC_SLSH,              MT(MOD_RSFT, JK_SB),
    LT(_KEY_EMU, JK_SB), KC_LALT,  MT(MOD_LGUI, JK_SB),          KC_SPC,                          MT(MOD_RGUI, JK_SB),  KC_RALT,  MO(_FN),    MT(MOD_RCTL, JK_SB)),
...
[_BASE_WINDOWS] = LAYOUT_60_iso( /* Base - Windows ISO */
    ...
    KC_LCTL,             KC_A,     KC_S,     KC_D,  KC_F,  KC_G,  KC_H,    KC_J,  KC_K,  KC_L,     KC_SCLN,              KC_QUOT,                KC_NUHS, KC_ENT,
    MT(MOD_LSFT, JK_SB), KC_NUBS,  KC_Z,     KC_X,  KC_C,  KC_V,  KC_B,    KC_N,  KC_M,  KC_COMM,  KC_DOT,               KC_SLSH,                         MT(MOD_RSFT, JK_SB),
    LT(_KEY_EMU, JK_SB), KC_LGUI,  MT(MOD_LALT, JK_SB),           KC_SPC,                          MT(MOD_RALT, JK_SB),  LT(_WIN_SPCL, KC_RGUI), MO(_FN), MT(MOD_RCTL, JK_SB)),
...
[_FN] = LAYOUT_60_iso( /* Keyboard Functions - Fn */
    ...
    KC_TRNS, EF_DEC,   EF_INC,   ES_DEC,   ES_INC,   BR_DEC,     BR_INC,   KC_TRNS,  MACOS,    KC_TRNS,  KC_TRNS,  KC_TRNS,            KC_MNXT,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  case WINDOWS:
    ...
    return false;
    // Shift Braces Tap
   case MT(MOD_LSFT, JK_SB):
     if (record->tap.count && record->event.pressed) {
       tap_code16(S(KC_8));
       return false;
     }
     break;
   case MT(MOD_RSFT, JK_SB):
     if (record->tap.count && record->event.pressed) {
       tap_code16(S(KC_9));
       return false;
     }
     break;
  case MT(MOD_LALT, JK_SB):
    if (record->tap.count && record->event.pressed) {
      tap_code16(RALT(KC_8));
      return false;
    }
    break;
  case MT(MOD_RALT, JK_SB):
    if (record->tap.count && record->event.pressed) {
      tap_code16(RALT(KC_9));
      return false;
    }
    break;
  case MT(MOD_LGUI, JK_SB):
    if (record->tap.count && record->event.pressed) {
      tap_code16(RALT(KC_5));
      return false;
    }
    break;
  case MT(MOD_RGUI, JK_SB):
    if (record->tap.count && record->event.pressed) {
      tap_code16(RALT(KC_6));
      return false;
    }
    break;
  case LT(_KEY_EMU, JK_SB):
    if (record->tap.count && record->event.pressed) {
